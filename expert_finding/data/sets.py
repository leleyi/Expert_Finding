...
class DataSet:
    def __init__(self, name, documents_file_path=None, experts_folder_path=None, version=None):
        ...
        if documents_file_path is not None and experts_folder_path is not None:
            if name == "aminer":
                ...
                self.getLExR(documents_file_path, experts_folder_path)
            elif name == "acm":
                self.getAcm(documents_file_path, experts_folder_path, self.version)
    ...
    def reduce_LExR(self, documents_set, candidates_set):
        print("reducing with documents_set", len(documents_set), "candidates_set", len(candidates_set))
        docs_mask = documents_set
        cands_mask = candidates_set
        cands_boolean_mask = np.zeros(len(self.ds.candidates), dtype=np.bool_)
    ...
    def clean_associations_LExR(self, max_documents_per_candidates=None, min_documents_per_candidates=None):
        num_docs_per_can = np.squeeze(np.asarray(self.ds.associations.sum(axis=0)))
    ...
    def getAminer(self, documents_file_path, experts_folder_path, version):
        ...
        experts = expert_finding.data.io_aminer.load_experts(experts_folder_path, version=version)
    ...
    def getAcm(self, documents_file_path, experts_folder_path, version):
        # Load data from disk
        print("Loading documents from aminer dataset...")
        documents = expert_finding.data.io_aminer.load_papers(documents_file_path)
        print("Loading experts from aminer dataset...")
    ...
    def getLExR(self, documents_file_path, expert_file_path):
        # Load data from disk
        print("Loading documents from aminer dataset...")
        documents = expert_finding.data.io_aminer.load_papers_LExR(documents_file_path)
        print("Loading experts from aminer dataset...")
        experts = expert_finding.data.io_aminer.load_expert_LExR(expert_file_path)

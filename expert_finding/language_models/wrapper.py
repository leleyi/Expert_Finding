...
#     impact_query_expert_finding.data.io.save_as_json(output_dir_temp, "texts.json", texts)
#
...
#     # impact_query_expert_finding.data.io.save_as_json(output_dir_temp, config["length_filter_filename"],
#     #                                                  corpus_length_filter)
#     # "length_filter.json"
#     impact_query_expert_finding.data.io.save_as_json(output_dir_temp, "length_filter.json", corpus_length_filter)
#
#     #  tf-idf
#     print("Compute Tfidf...")
...
#     impact_query_expert_finding.data.io.save_as_json(output_dir_temp, "stamp.json", stamp)
...
class LanguageModel:
    ...
    def compute_similarity(self, input_text):
        if self.type == "tf":
            token = tokenize(input_text, self.input_dir, grams=self.grams)
            dict = self.dictionary.doc2bow(token)
            print(self.corpus_index)
            indx = self.corpus_index[dict]
    ...
    def vectorize(self, docs):
        if self.type == "tf":
            sp = np.array(
                [self.dictionary.doc2bow(tokenize(input_text, self.input_dir, grams=self.grams)) for input_text in
                 docs])
            M = len(docs)
            ...
            sp = np.array(
                [self.tfidf[self.dictionary.doc2bow(tokenize(input_text, self.input_dir, grams=self.grams))] for
                 input_text in docs])
            M = len(docs)
            N = len(self.dictionary)
            dat = list()
